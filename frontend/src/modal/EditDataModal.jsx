...
const EditDataModal = ({ handleClose, visible, selectedId }) => {
	...
	async function editData(data){
		try {
			const response = await axiosInstance.patch(`/users/${selectedId}`, data );
	...
	async function getDataById(id){
		try{
			...
			//populate form field using setValue
	...
	useEffect(() => {
			getDataById(selectedId);
	}, []);
	
	return (
		<div className="fixed inset-0 flex bg-gray-500 bg-opacity-50  h-screen items-center justify-center select-none">
		
		<div className="flex flex-col bg-white rounded-lg shadow-md px-14 py-5 min-w-[30%] min-h-[30%] gap-5 mt-[-6rem] "> 
			<div className="font-bold text-center text-2xl">Edit data</div>
						...
						<Controller
							...
							render={({ field, fieldState }) => (
								<>
									...
									/>
									{ fieldState.error && (
										<p className="text-red-500 mt-1 text-md">{fieldState.error.message}</p>
									)}
								</>
							)}
						/>
						...
						<Controller 
							...
							render={({ field, fieldState }) => (
								<>
									...
									/>
									{ fieldState.error && (
										<p className="text-red-500 mt-1 text-[8px]">{fieldState.error.message}</p>
									)}
								</>
							)}
						/>
